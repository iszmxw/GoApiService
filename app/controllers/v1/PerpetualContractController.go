...
func (h *PerpetualContractController) TradeHandler(c *gin.Context) {
	...
	WalletStream, err4 := new(models.WalletStream).SetAddData("2", "2", "10", addData, Currency, UsersWallet)
	if err4 != nil {
		DB.Rollback()
...
func (h *PerpetualContractController) LiquidationHandler(c *gin.Context) {
	...
	*/
	...
	data.Type = "2"                                                     // 流转类型 1 币币交易 2 永续合约 3 期权合约 4 申购交易 5 划转 6 充值 7 提现 8 冻结
	data.TypeDetail = "10"                                              // 流转详细类型  1 USDT充值  2 银行卡充值  3 币币交易手续费  4 永续合约手续费  5 期权合约手续费  6 币币账户划转到合约账户  7 合约账户划转到币币账户  8 申购冻结  9 币币交易  10 永续合约  11 期权合约
